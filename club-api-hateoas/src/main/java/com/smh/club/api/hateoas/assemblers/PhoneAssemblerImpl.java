...
import com.smh.club.api.hateoas.controllers.PhoneController;
...
public class PhoneAssemblerImpl extends RepresentationModelAssemblerSupport<PhoneEntity, PhoneModel>
    ...
    public PhoneAssemblerImpl(PhoneMapper mapper, PagedResourcesAssembler<PhoneEntity> pagedAssembler) {
        super(PhoneController.class, PhoneModel.class);
        this.mapper = mapper;
    ...
    public PhoneModel toModel(@NonNull PhoneEntity entity) {
        ...
        model.add(linkTo(methodOn(PhoneController.class).update(entity.getId(), model)).withRel("update"));
        model.add(linkTo(methodOn(PhoneController.class).delete(entity.getId())).withRel("delete"));
        return model;
    }
